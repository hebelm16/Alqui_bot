...
async def obtener_informe_mensual(mes: int, anio: int) -> dict:
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            await cur.execute("SELECT SUM(monto) FROM pagos WHERE EXTRACT(MONTH FROM TO_DATE(fecha, 'DD/MM/YYYY HH24:MI')) = %s AND EXTRACT(YEAR FROM TO_DATE(fecha, 'DD/MM/YYYY HH24:MI')) = %s", (mes, anio))
            total_pagos_mes = (await cur.fetchone())[0] or 0.0

            # Total Gastos para el mes/año
            await cur.execute("SELECT SUM(monto) FROM gastos WHERE EXTRACT(MONTH FROM TO_DATE(fecha, 'DD/MM/YYYY HH24:MI')) = %s AND EXTRACT(YEAR FROM TO_DATE(fecha, 'DD/MM/YYYY HH24:MI')) = %s", (mes, anio))
            total_gastos_mes = (await cur.fetchone())[0] or 0.0

            # Pagos del mes
            await cur.execute("SELECT fecha, inquilino, monto FROM pagos WHERE EXTRACT(MONTH FROM TO_DATE(fecha, 'DD/MM/YYYY HH24:MI')) = %s AND EXTRACT(YEAR FROM TO_DATE(fecha, 'DD/MM/YYYY HH24:MI')) = %s ORDER BY id ASC", (mes, anio))
            pagos_mes = await cur.fetchall()

            # Gastos del mes
            await cur.execute("SELECT fecha, descripcion, monto FROM gastos WHERE EXTRACT(MONTH FROM TO_DATE(fecha, 'DD/MM/YYYY HH24:MI')) = %s AND EXTRACT(YEAR FROM TO_DATE(fecha, 'DD/MM/YYYY HH24:MI')) = %s ORDER BY id ASC", (mes, anio))
